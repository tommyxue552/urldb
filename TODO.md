...
- [x] 搜索、点击、渠道归因统计和速率限制。
- [x] Meilisearch 运维、索引同步与失败回退监控。

## P3

- [x] Provider 契约测试、合规审计报表、运营看板。
- [x] 评估是否需要迁移到显式队列/worker：当前无任务积压，决定暂缓。详见 `docs/task-operations.md`。
- [x] 增加任务量、积压年龄、任务耗时和恢复次数的管理员运维快照接口，作为后续迁移评估基线。
- [ ] 根据实际任务量、延迟、并发数和副本要求，实施具有指标、持久化任务认领、租约恢复与幂等约束的队列/worker。
